...
def _df_to_md(df: pd.DataFrame, index=False) -> str:
    """Devuelve una tabla Markdown. Si falta 'tabulate', usa un bloque de código plano."""
    try:
...
def run_demografia(save_md: bool = True) -> dict:
    ...
    if grado_col:
        ...
        (df[grado_col].astype(str).str.strip()
         .replace({"nan": pd.NA}).dropna()
         .value_counts().head(15)
         .plot(kind="bar", edgecolor="black"))
        plt.title("Top 15 grados/rangos")
        plt.xlabel("Grado/Rango")
        plt.ylabel("Cantidad")
        plt.xticks(rotation=45, ha="right")
        plt.tight_layout()
        plt.savefig(fig3, dpi=120)
        plt.close()
...
def build_resumen():
    ...
    resumen = [
        ...
        f"_Última ejecución: {dt.datetime.now():%Y-%m-%d %H:%M}_",
        "",
        "## Enlaces a reportes",
